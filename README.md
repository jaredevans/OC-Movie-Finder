...
## Features
- Searchable list of Open Caption screenings with date/time and theater details
...
- **Storage**: Results are persisted in a local SQLite DB (`server/movies.db`).
...
3. From the project root, run both server and client: `npm run dev`
...
- Supported theaters (as of now): AMC Montgomery 16, AMC Georgetown 14, Regal Rockville Center.

## API reference
- `GET /api/movies` – returns all stored OC showings sorted by showtime.
- `GET /api/movies?q=term` – search by movie title or theater.
